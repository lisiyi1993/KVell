...
int main(int argc, char **argv) {
   ...
   // char input_sql[] = "SELECT n_NATIONNAME , SUM(l_EXTENDEDPRICE) AS revenue FROM customer , orders , lineitem , nation , region WHERE c_CUSTKEY = o_CUSTKEY AND o_ORDERKEY = l_ORDERKEY AND o_ORDERDATE >= 2021-03-01 AND o_ORDERDATE < 2021-06-01 AND c_NATIONKEY = n_NATIONKEY AND n_REGIONKEY = r_REGIONKEY AND r_REGIONNAME = Asia GROUP BY n_NATIONNAME ORDER BY revenue DESC";
